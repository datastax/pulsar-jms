...
public interface JMSAdmin {
  ...
  /**
   * Get the handle to a Queue. A {@link Session#createQueue(String)} this method does not fail in
   * case the destination doesn't exist.
   *
   * @param queue
   * @return the handle
   * @throws JMSException
  ...
  /**
   * Get the handle to a Topic. A {@link Session#createQueue(String)} this method does not fail in
   * case the destination doesn't exist.
   *
   * @param topic
   * @return the handle
  ...
  /**
   * Create a new Pulsar topic and set-up it as a Queue. A JMS Queue maps to a Pulsar Subscription.
   * Create the topic if it does not exist. Create the subscription if it does not exit. If the
   * topic already exists the API fails in case it doesn't match the expected number of partitions.
   ...
   * @throws JMSException
   */
  void createQueue(Queue queue, int partitions, boolean enableFilters, String selector)
      throws JMSException;
  ...
  /**
   * Set the filter on a Queue. The name of the subscription is defined by the Destination
   * ("jms-queue" is the default name).
   *
   * @param destination the destination
   * @param enableFilters enable the filters
   ...
   * @param enableFilters enable the filters
   * @param selector the selector
   * @param fromBeginning start the subscription from the beginning of the topic, otherwise it will
   *     start from the tail.
   * @throws JMSException
