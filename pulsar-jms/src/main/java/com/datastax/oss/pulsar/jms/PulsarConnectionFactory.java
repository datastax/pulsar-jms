...
public class PulsarConnectionFactory
  ...
  private transient boolean useTemporaryProducers = false;
  ...
  private synchronized void ensureInitialized(String connectUsername, String connectPassword)
    ...
    try {
      ...
      this.useTemporaryProducers =
          Boolean.parseBoolean(
              getAndRemoveString("jms.useTemporaryProducers", "false", configurationCopy));
  ...
  public void close() {
    ...
    for (Producer<?> producer : getProducers().values()) {
      try {
        producer.close();
  ...
  private String computeKeyFromTopicName(
    ...
    if (pulsarMessageProducer instanceof PulsarMessageTemporaryProducer) {
      return ((PulsarMessageTemporaryProducer) pulsarMessageProducer).getUniqueID();
    }
  ...
  public synchronized boolean isUseTemporaryProducers() {
