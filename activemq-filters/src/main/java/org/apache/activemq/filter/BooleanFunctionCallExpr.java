...
/**
 * Function call expression that evaluates to a boolean value. Selector parsing requires
 * BooleanExpression objects for Boolean expressions, such as operands to AND, and as the final
 * result of a selector. This provides that interface for function call expressions that resolve to
 * Boolean values.
 *
 * <p>If a function can return different types at evaluation-time, the function implementation needs
 * to decide whether it supports casting to Boolean at parse-time.
 *
...
public class BooleanFunctionCallExpr extends FunctionCallExpression implements BooleanExpression {
  /**
   * Constructs a function call expression with the named filter function and arguments, which
  ...
  /**
   * Evaluate the function call expression, in the given context, and return an indication of
   * whether the expression "matches" (i.e.&nbsp;evaluates to true).
   *
