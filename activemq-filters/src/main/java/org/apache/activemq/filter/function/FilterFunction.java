...
/**
 * Interface required for objects that will be registered as functions for use in selectors. Handles
 * parse- time and evaluation-time operations.
 */
public interface FilterFunction {
  /**
   * Check whether the function, as it is used, is valid. Checking arguments here will return errors
   * to clients at the time invalid selectors are initially specified, rather than waiting until the
   * selector is applied to a message.
   *
   ...
   * expression parsing after the full expression for the function call, including its arguments,
   * has been parsed. This allows functions with variable return types to function as boolean
   * expressions in selectors without sacrificing parse-time checking.
   *
   * @param expr - the full expression of the function call, as used.
  ...
  /**
   * Evaluate the function call in the given context. The arguments must be evaluated, as-needed, by
   * the function. Note that boolean expressions must return Boolean objects.
   *
   * @param expr - the full expression of the function call, as used.
