 ...
 *
 * <p>Multiple Expression/ExpressionListener pairs can be added to a MultiExpressionEvaluator
 * object. When the MultiExpressionEvaluator object is evaluated, all the registed Expressions are
 * evaluated and then the associated ExpressionListener is invoked to inform it of the evaluation
 * result.
 *
 * <p>By evaluating multiple expressions at one time, some optimizations can be made to reduce the
 * number of computations normally required to evaluate all the expressions.
 *
 * <p>When this class adds an Expression it wrapps each node in the Expression's AST with a
 * CacheExpression object. Then each CacheExpression object (one for each node) is placed in the
 * cachedExpressions map. The cachedExpressions map allows us to find the sub expressions that are
 * common across two different expressions. When adding an Expression in, if a sub Expression of the
 * Expression is allready in the cachedExpressions map, then instead of wrapping the sub expression
 * in a new CacheExpression object, we reuse the CacheExpression allready int the map.
 *
 * <p>To help illustrate what going on, lets try to give an exmample: If we denote the AST of a
 * Expression as follows: [AST-Node-Type,Left-Node,Right-Node], then A expression like: "3*5+6"
 * would result in "[*,3,[+,5,6]]"
 *
 * <p>If the [*,3,[+,5,6]] expression is added to the MultiExpressionEvaluator, it would really be
 * converted to: [c0,[*,3,[c1,[+,5,6]]]] where c0 and c1 represent the CacheExpression expression
 * objects that cache the results of the * and the + operation. Constants and Property nodes are not
 * cached.
 *
 * <p>If later on we add the following expression [=,11,[+,5,6]] ("11=5+6") to the
 * MultiExpressionEvaluator it would be converted to: [c2,[=,11,[c1,[+,5,6]]]], where c2 is a new
 * CacheExpression object but c1 is the same CacheExpression used in the previous expression.
 *
 * <p>When the expressions are evaluated, the c1 CacheExpression object will only evaluate the
 ...
 *
 * <p>Problems: - cacheing the values introduces overhead. It may be possible to be smarter about
 * WHICH nodes in the AST are cached and which are not. - Current implementation is not thread safe.
 * This is because you need a way to invalidate all the cached values so that the next evaluation
 * re-evaluates the nodes. By going single threaded, chache invalidation is done quickly by
 * incrementing a 'view' counter. When a CacheExpressionnotices it's last cached value was generated
 * in an old 'view', it invalidates its cached value.
 *
 * <p>$Date: 2005/08/27 03:52:36 $
 */
public class MultiExpressionEvaluator {
  ...
  /**
   * A UnaryExpression that caches the result of the nested expression. The cached value is valid if
   * the CacheExpression.cview==MultiExpressionEvaluator.view
   */
  public class CacheExpression extends UnaryExpression {
    ...
    /** @see Expression#evaluate(MessageEvaluationContext) */
  ...
  /**
   * Adds an ExpressionListener to a given expression. When evaluate is called, the
   * ExpressionListener will be provided the results of the Expression applied to the evaluated
   * message.
   */
  ...
  /**
   * Finds the CacheExpression that has been associated with an expression. If it is the first time
   * the Expression is being added to the Cache, a new CacheExpression is created and associated
   * with the expression.
   *
   * <p>This method updates the reference counters on the CacheExpression to know when it is no
   * longer needed.
   */
  ...
  /**
   * Removes an expression from the cache. Updates the reference counters on the CacheExpression
   * object. When the refernce counter goes to zero, the entry int the Expression to CacheExpression
   * map is removed.
   *
   * @param cn
   */
   ...
   * ExpressionListeners are notified of the result of the evaluation.
   *
   * @param message
   */
  public void evaluate(MessageEvaluationContext message) {
    ...
    for (Iterator<ExpressionListenerSet> iter = expressionListeners.iterator(); iter.hasNext(); ) {
      ...
      try {
        ...
        for (Iterator<ExpressionListener> iterator = els.listeners.iterator();
            iterator.hasNext(); ) {
