...
/**
 * Function which splits a string into a list of strings given a regular expression for the
 * separator.
 */
public class splitFunction implements FilterFunction {
  ...
  public boolean isValid(FunctionCallExpression expr) {
    if ((expr.getNumArguments() >= 2) && (expr.getNumArguments() <= 3)) return true;
  ...
  /**
   * Evaluate the given expression for this function in the given context. A list of zero or more
   * strings results from the evaluation. The result of the evaluation of the first argument is
   * split with the regular expression which results from the evaluation of the second argument. If
   * a third argument is given, it is an integer which limits the split. String#split() performs the
   * split.
   *
   * <p>The first two arguments must be Strings. If a third is given, it must be an Integer.
   *
   * @param expr - the expression consisting of a call to this function.
