 ...
 * as-needed. For example, REGEX( 'AA', 'XAAX' ) evaluates to true while REGEX( '^AA$' , 'XAAX' )
 * evaluates to false.
 */
public class regexMatchFunction implements FilterFunction {
  ...
  /**
   * Check whether the given expression is a valid call of this function. Two arguments are
   * required. When evaluated, the arguments are converted to strings if they are not already
   * strings.
   *
  ...
  public boolean isValid(FunctionCallExpression expr) {
    if (expr.getNumArguments() == 2) return true;
  ...
  /**
   * Evalutate the given expression, which consists of a call to this function, in the context
   * given. Returns an indication of whether the second argument matches the regular expression in
   * the first argument.
   *
   * @param expr - the expression consisting of a call to this function.
  ...
  public Object evaluate(FunctionCallExpression expr, MessageEvaluationContext message)
    ...
    if (reg != null) {
      ...
      if (reg instanceof String) reg_str = (String) reg;
      else reg_str = reg.toString();
      ...
      if (cand != null) {
        // Convert to a string, if it's not already a string.
        if (cand instanceof String) cand_str = (String) cand;
        else cand_str = cand.toString();
  ...
  /**
   * Retrieve a compiled pattern for the given pattern string. A cache of recently used strings is
   * maintained to improve performance.
   *
