 ...
 * example, INLIST('1,2,3', '1'), will cause an exception to be thrown at evaluation-time.
 */
public class inListFunction implements FilterFunction {
   ...
   * required. Note that the evaluated results of the arguments will be compared with
   * Object#equals().
   *
  ...
  public boolean isValid(FunctionCallExpression expr) {
    if (expr.getNumArguments() != 2) return false;

    return true;
  }

  /**
   * Check whether the given expression, which consists of a call to this function, evaluates to a
   * Boolean. If the function can return different more than one type of value at evaluation-time,
   * it must decide whether to cast the result to a Boolean at this time.
   *
   * @param expr - the expression consisting of a call to this function.
   * @return true - if the expression is valid; false - otherwise.
   */
   ...
   * given. Checks whether the second argument is a member of the list in the first argument.
   *
   * @param expr - the expression consisting of a call to this function.
